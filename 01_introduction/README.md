...
- 人工智能 AI：整个学科的名称，类似于“生物学”或“化学”，通过机器模拟人类认知 – 感知、学习、推理、决策 。
- 数据科学/挖掘 Data Mining：数据科学（Data Sciense） = 数据 + 科学，是从数据中获取信息的科学，其交叉领域包括计算机、统计及专业知识。它利用机器学习和统计学等技术，从大量数据中提取有效信息、模型。
- 统计：从数据中获取有用的信息，用于数理统计和应用统计（描述统计、推论统计）。
- 机器学习 ML：是人工智能的重要组成部分，但不是唯一的部分。它研究和构建的是一种特殊算法，能够让计算机从经验数据中产生“模型”，基于模型对新的数据进行预测。
...
机器学习的目标是基于输入的数据来预测结果，所有的机器学习任务都可以用这种方式来表示，否则从一开始它就不是个机器学习问题。样本越是多样化，越容易找到相关联的模式以及预测出结果。因此，需要 3 个部分来训练机器：
...
学习（learning）/训练（trianing）指从数据集中学得模型的过程。可以把 ML 看成在假设空间 $H$ 中搜索的过程，搜索的目的是找到与训练集“匹配（fit）”（性能最好）的假设。ML 就是用“数据集 D”从“假设空间 H”中计算与“目标函数 f”最接近的“最优解 g”的过程，具体步骤见下一章。
...
根据“天下无免费午餐 NFL 定理”，既便“猛炸天”比“胡逼猜”模型表现要好，但目标函数 f 是未知的，完美预测样本内数据对预测样本外数据没有什么帮助。只有样本外的表现好才能证明机器真正学习到了目标函数 f。但样本外又是未知的，机器如何学习呢？
...
既然找不到 f 就来找它的替身 g，g 函数是从假设空间 H 中通过机器学习得出的最优假设，既 g 是 f 的替身（g ≈ f）。具体原理如下图：
...
- 误差：模型在训练集上的误差称为“训练误差”或“经验误差”，在新样本上的误差称为“泛化误差”。机器学习的目的是希望得到泛化误差小的模型。但我们事先并不知道新样本是什么样的，实际能做的是努力使经验误差最小化。
- 性能：在机器学习过程中，有多种学习算法可以选择，同一个学习算法也有众多参数可以选择。学习算法的选择是通过对候选学习算法得到的模型的泛化误差进行评估，选择泛化误差最小的那个模型对应的学习算法及参数。对模型的泛化性能进行评估，需要有衡量泛化能力的评价标准，也就是性能度量。评估的指标主要有准确率、召回率、F值，这个过程可以看到模型如何对尚未看到的数是如何做预测的，这意味着代表模型对潜在样本的表现。主要的性能度量包括：错误率、精度、查准率、查全率、F1、ROC、AUC。

#### 性能评估指标 vs. 损失函数

- 性能评估指标（metrics）：对整个模型，理论上希望对模型性能进行的评估方式，学习算法优化时用于评估训练所得模型的整体性能，用于优化学习算法及其超参。a function to evaluate the quality of model
- 损失函数：对某一次训练，在训练中用来对候选模型的性能进行的评估方式，模型训练时通过损失函数来确定优化的方向。然后选择泛化误差最小的那个作为最优模型（best function）。损失函数必须是可求梯度的，就是可以求偏导数的，这样比较容易被优化。
- 评估函数（eval function）：不需要被优化。
...
- 偏差 bias（准心）：度量了模型的期望预测与真实结果的偏离程度，反映了模型本身的**拟合能力**。
- 方差 variance（稳定性）：度量了同样大小的训练集的变动所导致模型性能的变化，刻画了数据扰动造成的影响，也就是**欠拟合能力**。
