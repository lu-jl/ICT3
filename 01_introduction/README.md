...
- 假设函数 h（hypothesis）：从样本空间 $X$ 到标记空间 $Y$ 的某一种映射。由于不能确定 h 是否真是目标函数 f，因此才会被称为假设。
...
学得模型（最优函数）在数据上表现的量化形式，选取合适的函数来表示什么样子的模型是好的。也就是针对每个假设 h 的评估，用来判断该假设是否与数据集匹配，然后选择性能最好（泛化误差最小）的那个假设作为最优函数 g。

#### 性能度量/误差

性能度量（performance measure）是从业务的角度来评判模型的方式。它是关于真实值和预测值的关系，真实值与预测值越接近，或者说真实的分布与预测分布越接近，性能越好。
...
损失函数是为了达到业务的性能度量所设定的优化目标，是实现性能度量的实际方式。

机器学习中用损失函数来评估假设 h 针对某一个样本的性能，也就是计算模型预测值与真实值的差距，从而确定优化的方向。损失函数必须是可求梯度的，就是可以求偏导数的，这样比较容易被优化。

- 损失函数（Loss Function）：指在单个样本上的误差，用来评估模型一次预测的好坏，可表示为：$L(y_i,f(x_i))$。
- 代价函数（Cost Function）：又叫成本函数，是对数据集所有样本点的损失函数的平均值，可表示为：$\frac{1}{N}\sum^N_{i=1}L(y_i,f(x_i))$。习惯上所说的“损失函数”其实有时候是指“代价函数”。

- 目标函数（Object Function）：指最终需要优化的函数，可以表示为：$\frac{1}{N}\sum^N_{i=1}L(y_i,f(x_i))+\lambda J(f)$，其中$J(f)$ 为正则项。

##### vs. 梯度下降法

梯度下降法是一个使损失函数越来越小的优化算法。通过梯度下降法训练的过程，就是通过不断的迭代计算，使用梯度下降的优化算法，使得单个样本的损失函数越来越小。损失函数越小就表示算法达到意义上的最优。
...
- Squared Error：
- MSE（Mean Squared Error）均方误差：线性回归的损失函数
- 均方根误差（RMSE）
- 平均绝对误差（MAE）
- 均方对数误差（MSLE）
- 均方根对数误差（RMSLE）
...
损失函数也与性能度量类似，真实值与预测值差别越大、Loss越大，优化的目标就是减小Loss。从评估的角度来说，性能度量是整体业务目标，而损失函数是具体优化方向，两者可以相同也可以不同。常见的 MSE 均方误差或 logLoss 既可以被当作性能度量，也是回归问题的损失函数。但在更多的问题中，经常发现为了减小模型的错误率（性能度量），并不直接优化错误率而是会优化另一个函数（损失函数）。【4】例如：
...
完成“表示”和“评估”后要做的就是优化，对评估函数进行求解，找出最合适的解来确定最终的模型。可以把 ML 看成在假设空间 $H$ 中搜索的过程，搜索的目的是找到与训练集“匹配（fit）”（性能最好）的假设 h。ML 就是用“数据集 D”从“假设空间 H”中计算与“目标函数 f”最接近的“最优函数 g”的过程。

机器学习模型 = 假设空间 H + 学习算法 A。

- 学习算法 A：H 是学习算法 A 自认为可能的映射的集合，而 f 是真正满足要求的映射（上帝视角），学习训练的目的是在 H 中寻找接近 f 的函数 g，使得 $g≈f$。
  - 学习算法选择：在机器学习过程中，有多种学习算法可以选择，同一个学习算法也有众多参数（超级参数）可以选择。学习算法的选择是通过对候选学习算法学得的模型的泛化性能进行评估，选择泛化性能最好的那个模型对应的学习算法及超参。对模型的泛化性能进行评估，也就是利用损失函数来衡量模型的泛化性能。这个过程可以看到模型如何对尚未看到的数据做预测的，这意味着代表模型对潜在样本的表现。
  - 优化方式：对损失函数进行求解，找出最合适的解，来确定最终的模型（最优函数）。
- 归纳偏好：归纳偏好表示在学习过程中对某一类假设的偏好，它可以看做是“学习算法”自身的一个在很庞大的假设空间中对假设进行选择的启发式“价值观”。
