...
- 性能评估指标（metrics）：理论上希望对模型性能进行的评估方式，学习算法优化时用于评估训练所得模型的整体性能，用于优化学习算法及其超参。a function to evaluate the quality of model
- 损失函数：在训练中用来对候选模型的性能进行的评估方式，，模型训练时通过损失函数来确定优化的方向。然后选择泛化误差最小的那个作为最优模型（best function）。损失函数必须是可求梯度的，就是可以求偏导数的，这样比较容易被优化。
