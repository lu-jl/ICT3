...
- 统计：从数据中获取有用的信息，用于数理统计和应用统计（描述统计、推论统计）。
- ML 机器学习：是人工智能的重要组成部分，但不是唯一的部分。它研究和构建的是一种特殊算法，能够让计算机从经验数据中产生“模型”，基于模型对新的数据进行预测。
...
- 第二次黄金期：1980 年，第一次 AI 冬天，Hopfield 网络&BP 算法，第五代计算机兴起。第五代计算机失败，DARPA削减投入。2000年，第二次AI冬天。
...
在“监督学习”中，有一个“监督者”或者“老师”提供给机器所有的答案来辅助学习，比如图片中是猫还是狗。“老师”已经完成数据集的划分——标注“猫”或“狗”，机器就使用这些示例数据来学习，逐个学习区分猫或狗。
...
无监督学习比有监督学习出现得稍晚——在上世纪90年代，这类算法用的相对较少，有时候仅仅是因为没得选才找上它们。有标注的数据是很奢侈的。假设要创建一个——比如说“公共汽车分类器”，那是不是要亲自去街上拍上几百万张该死的公共汽车的照片，然后还得把这些图片一一标注出来？这需要花费大量的时间！可以尝试使用无监督学习。无监督学习通常用于探索性数据分析（exploratory data analysis），而不是作为主要的算法，它基于未知类别的训练数据进行学习。
...
最接近人类学习的办法。即有 input，取代 label 的是 reward——不告诉你正确答案是什么，但是会根据你的答案表扬你或者批评你。
...
机器学习的目标是基于输入的数据来预测结果，所有的机器学习任务都可以用这种方式来表示，否则从一开始它就不是个机器学习问题。样本越是多样化，越容易找到相关联的模式以及预测出结果。

#### 数据集

想检测垃圾邮件，就需要获取垃圾信息的样本。想预测股票，就要找到历史价格信息。想找出用户偏好，就要分析他们在Facebook上的活动记录。数据越多样化，结果越好。对于拼命运转的机器而言，至少也得几十万行数据才够。获取数据有两种主要途径：手动或者自动。手动采集的数据混杂的错误少，但要耗费更多的时间。自动化的方法相对便宜，可以搜集一切能找到的数据，但数据质量往往不够好。样本就是通常说的“训练数据”，包括输入和结果两部分。样本的数量和质量对机器学习的效果至关重要，如果样本量太少或样本分布不均衡，对训练出来的模型就有很大的影响。

- 样本：给定样本集 $\hat{D}={(x_1,y_1),(x_2,y_2),\dots,(x_m,y_m)},x_i \in X$，假设 $X$ 中的所有样本服从一个隐含未知的分布 $D$，$\hat{D}$ 中所有样本都是独立地从这个分布上采样而得，即满足独立同分布 IID（Independent and Identically Distributed）。

#### 目标函数

- 目标函数 f：也被称为概念 Concept，它是从样本空间 $X$ 到标记空间 $Y$ 的映射，它决定样本 x 的真实标记 y。

#### 假设函数

- 假设 h（hypothesis）：从样本空间 $X$ 到标记空间 $Y$ 的某一种映射，由于不能确定 h 是否真是目标函数 f，因此才会被称为假设。
  - 假设空间 H（Hypothesis Space）：所有可能的假设的集合，$h\in H$。

H 中的假设 h 可能是一个条件概率分布，也可能是一个决策函数。所有 h 的可能结果构成了模型的假设空间 H。很多时候 H 的函数类型是明确的，需要计算的是函数的参数，比如确定 f 函数为一个线性函数，那么 h 的函数表示就可以写为：$y=a_1 x+a_0$。

- 最优假设 g：训练从 H 中所得的最优函数，它是目标函数 f 的替身。

#### 损失函数

对某一次训练，在训练中用来对候选模型的性能进行的评估方式，模型训练时通过损失函数来确定优化的方向。然后选择泛化误差最小的那个作为最优假设 g（有时也被称为 best function）。损失函数必须是可求梯度的，就是可以求偏导数的，这样比较容易被优化。

- 误差（Error）：误差被定义为模型的实际预测输出与样本真实输出之间的差异。
  - 训练误差（training error）/经验误差（empirical error）：指学到的模型在训练数据集上的误差。训练误差描述输入属性与输出分类之间的相关性，能够判定给定的问题是不是一个容易学习的问题。
    - 在样本上计算而得的平均误差 $\hat{E}(h;\hat{D})=\frac{1}{m}\displaystyle \sum_{i=1}^{m}I(h(x_i)\neq y_i)$
  - 测试误差（testing error）/泛化误差：指学到的模型在新的测试样本上的误差。测试误差反映了学到的模型对未知的测试数据集的预测能力，学习的目的是得到测试误差小的模型。
    - 在其全部取值范围内所有数据的数学平均误差 $E(h;D)=P_{x\in D}(h(x)\neq y)$
  - ？？？由于 $\hat{D}$ 是 D 的独立同分布采样，因此 h 的经验误差的期望等于其泛化误差。
  - 错误率：在分类问题中，分类错误的样本占全部样本的比例。

#### 学习算法

学习（learning）/训练（trianing）指从数据集中学得模型的过程。可以把 ML 看成在假设空间 $H$ 中搜索的过程，搜索的目的是找到与训练集“匹配（fit）”（性能最好）的假设。ML 就是用“数据集 D”从“假设空间 H”中计算与“目标函数 f”最接近的“最优函数 g”的过程，具体步骤见下一章。

- 学习算法 A：H 是学习算法 A 自认为可能的映射的集合，而 f 是真正满足要求的映射的集合（上帝视角），学习训练的目的是在 H 中寻找寻找接近 f 的函数 g，使得 $g≈f$。
...
<img src="figures/image-20210705101203436.png" alt="image-20210705101203436" style="zoom: 33%;" />

需要注意一点：如果数据质量差，即使采用最好的算法也无济于事，这被称为“垃圾进，垃圾出”（garbae in garbage out，GIGO）。所以，在把大量心思花到正确率之前，应该获取更多的数据。
...
- 样本外数据预测：学习的最终目的就是找到一个未知的目标函数 f，因为它可以将任意数据都能正确分类。但任意数据还包括样本外的未知数据，因此针对样本外数据的目标函数 f 很难学得。

既然找不到 f 就来找它的替身 g，g 函数是从假设空间 H 中通过机器学习算法 A 得出的最优假设，既 g 是 f 的替身（g ≈ f）。具体原理如下图：

<img src="figures/image-20210328141011859.png" alt="image-20210328141011859" style="zoom: 25%;" />

训练集服从目标函数 f 的分布。学习算法 A 就是要从假设空间 H 中寻找一个最优的函数 g，使得样本空间的输入 X 经过该函数的映射得到的 g(X)，和真实的 Y 值之间的距离最小。这个最优的函数通常没办法直接计算得到，即没有解析解，需要用数值计算的方法不断迭代求解。因此如何寻找到 g 函数的全局最优解，以及使寻找过程尽量高效，就构成了机器学习的算法。如何保证 g 函数的参数空间最接近最优解，就是算法的策略。机器学习中用损失函数来评估模型的性能，也就是是否最接近最优解。损失函数用来计算模型预测值与真实值的差距，常用的有 0-1 损失函数、平方损失函数、绝对损失函数、对数损失函数等。
...
现在已经有了机器学习的框架，目标变成找到一个 g ≈ f。如果找得到 g 那么机器学习可行，找不到 g 那么机器学习不可行。而找到 g 的逻辑变为：**使 g 的泛化误差很小**。因为 f 的泛化误差理论上为 0，如果 g 的泛化误差很小，则证明了 g ≈ f，从而间接找到 f 的替身 g。得到 g 也就是学习到了目标函数 f，从而证明机器学习可行。

至此核心问题变为了寻求 g 的泛化误差。但因为目标函数 f 是未知的，从而使 g 的泛化误差无法计算。因此满足以下 2 点，则可以推导出 g 的泛化误差很小：
...
- 证明在某种条件下泛化误差近似经验误差 $E(g)≈\hat{E}(g)$ 
- 使得经验误差 $\hat{E}(g)$ 很小 
...
PAC（Probably Approximately Correct）的核心观点是：**当样本数足够大**时，样本的分布 v（对应 $\hat{E}(g)$）和总体的分布 u（对应 $E(g)$）就足够靠近。令误差参数 $\epsilon$ 为泛化误差 $E(h)$ 的上限，即 $E(h)\leq \epsilon$，表示学得模型 $h$ 所应满足的误差要求。具体的 Hoeffding 不等式为：<img src="figures/image-20210328155125291.png" alt="image-20210328155125291" style="zoom:25%;" />。
...
给定训练集 D，希望学习算法 A 学得的模型所对应的假设 g 尽可能接近目标函数 f。希望以比较大的把握学得比较好的模型，也就是以较大的概率学得误差满足预设上限的模型。

- PAC 辨识：对于 $0<\epsilon,\delta<1$，所有 $f\in F$ 和分布 $D$，若存在学习算法 A，其输出假设 $h\in H$ 满足：$P(E(h)\leq \epsilon)\geq 1-\delta$，则称学习算法 A 能从假设空间 H 中 PAC 辨识目标函数 f。这样的学习算法 A 能以较大的概率（至少$1-\delta$）学得目标函数 f 的近似（误差最多为 $\epsilon$）。
...
- PAC 可学习：令 m 表示从分布 $\hat{D}$ 中 IID 采样得到的样本数目。若存在学习算法 A 和多项式函数 poly(.,.,.)，使得对于任何 $m \geq poly(1/\epsilon, 1/\delta, size(x), size(c))$，A 能从假设空间 H 中 PAC 辨识目标函数 f，则称目标函数 f 对假设空间 H 而已是 PAC 可学习的。
- PAC 学习算法：若学习算法 A 使目标函数 f 为 PAC 可学习的，且 A 的运行时间也是多项式函数 $poly(1/\epsilon, 1/\delta, size(x), size(c))$，则称目标函数 f 是高效 PAC 可学习的，称 A 为目标函数 f 的 PAC 学习算法。
- 样本复杂度：满足 PAC 学习算法 A 所需的 $m\geq poly(1/\epsilon, 1/\delta, size(x), size(c))$ 中最小的 m，称为学习算法 A 的样本复杂度。

PAC 学习给出了一个抽象地刻画学习能力的框架。到底需要多少样本才能学得目标函数 f 的有效近似？对于 PAC 来说，只要训练集 D 的规模能让学习算法 A 以概率 $1-\delta$ 找到目标假设的 $\epsilon$ 近似即可。

#### g 的经验误差≈0

##### 从单一到有限

与之前的流程相比，目前把 g 用到训练集上来计算经验误差，如果很小那么验证出 g 学到了 f，如果很大那么验证出 g 没学到 f，但这本身不是学习只是验证。

<img src="figures/image-20210328183054301.png" alt="image-20210328183054301" style="zoom:25%;" />

因此当前的问题转变为：如何有一个机制使得 g 的经验误差 $\hat{E}(g)≈0$。也就是从假设空间 H 中选出一个 h 使得 $\hat{E}(h)$ 最小，并将这个最优假设设定为 g。只要满足 $P(E(g)>\epsilon)\leq \delta$，那么 $E(g)≈0$，g 就真正学到了 f。
...
- 误差：模型在训练集上的误差称为“训练误差”或“经验误差”，在新样本上的误差称为“泛化误差”。机器学习的目的是希望得到泛化误差小的模型。但因为事先并不知道新样本是什么样的，实际能做的是努力使经验误差最小化。
  - 损失函数：
- 性能：在机器学习过程中，有多种学习算法可以选择，同一个学习算法也有众多参数可以选择。学习算法的选择是通过对候选学习算法得到的模型的泛化误差进行评估，选择泛化误差最小的那个模型对应的学习算法及参数。
  - 性能评估指标（metrics）：优化学习算法 A 时用于评估训练所得模型的整体性能，用于优化学习算法及其超参。对模型的泛化性能进行评估，需要有衡量泛化能力的评价标准，也就是性能度量。这个过程可以看到模型如何对尚未看到的数是如何做预测的，这意味着代表模型对潜在样本的表现。

并且对于同一个训练流程，寻能评估指标与损失函数可以相同也可以不同。

- 相同：回归中的性能评估指标和损失函数都是 MSE 或 logLoss
- 不同：例如分类问题的损失函数可以是 cross entropy，但是评估函数可以是accuracy recall这些。
...
为了达到泛化误差最小的目的，机器学习应尽可能从训练样本中学出适用于新样本的“普遍规律”。当模型学得“太好”时，就会把训练样本自身的特点作为潜在样本都会具有的一般性质，这样就会导致泛化性能下降，被称为“过拟合”。常见的过拟合原因是学习能力过于强大。而对训练样本的一般性质尚未学好，则被称为“欠拟合”。常见的欠拟合原因是由于学习能力低下造成的。

#### 偏差/方差/噪声

$E(f;D)=bias^2(x)+var(x)+\epsilon ^2$：泛化误差可以分解为偏差、方差和噪声之和。

- 偏差 bias（准心）：度量了模型的期望预测与真实结果的偏离程度，反映了模型本身的拟合能力。
- 方差 variance（稳定性）：度量了同样大小的训练集的变动所导致模型性能的变化，刻画了数据扰动造成的影响，也就是欠拟合能力。
- 噪声：表达了在当前任务上任何模型所能达到的期望泛化误差的下线，即刻画了学习问题本身的难度。
...
如果说造成过拟合的原因是学习能力太强，造成欠拟合（underfitting）的原因就是学习能力太弱，以致于训练数据的基本性质都没能学到。
...
过拟合（overfitting）是把训练数据自身的一些特点错当做整体样本都具有的一般性质，这样会导致泛化能力下降。例如，如果接触的外国人较少，从没见过双眼皮的韩国人，思维中就难免出现“单眼皮都是韩国人”的错误定式。过拟合出现的原因通常是学习时模型包含的参数过多，从而导致训练误差较低但测试误差较高。
...
- 编程语言：推荐用于数据科学的编程语言是Python、R、Scala 和 Java。了解他们中的任何一个都可以，并且可以解决问题。 
