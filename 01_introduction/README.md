...
- 目标函数 f：也被称为概念 Concept，它是从样本空间 $X$ 到标记空间 $Y$ 的映射，它决定样本 x 的真实标记 y，也就是机器学习想学到的。
...
- 最优函数 g：训练从 H 中所得的最优函数，它是目标函数 f 的替身，有时也被称为学得的模型。
...
机器学习模型 = 假设空间 H + 学习算法 A。

- 学习算法 A：H 是学习算法 A 自认为可能的映射的集合，而 f 是真正满足要求的映射的集合（上帝视角），学习训练的目的是在 H 中寻找接近 f 的函数 g，使得 $g≈f$。
- 性能度量（学习算法选择）：针对每个假设 h 的评估，用来判断该假设是否与训练集匹配。在机器学习过程中，有多种学习算法可以选择，同一个学习算法也有众多参数（超级参数）可以选择。学习算法的选择是通过对候选学习算法得到的模型的泛化性能进行评估，选择泛化性能最好的那个模型对应的学习算法及超参。对模型的泛化性能进行评估，需要有衡量泛化性能的评价标准，也就是性能度量。这个过程可以看到模型如何对尚未看到的数据做预测的，这意味着代表模型对潜在样本的表现。
...
对某一次训练，在训练中用来对候选模型的性能进行的评估方式，模型训练时通过损失函数来确定优化的方向。然后选择泛化误差最小的那个作为最优函数 g（有时也被称为 best function）。损失函数必须是可求梯度的，就是可以求偏导数的，这样比较容易被优化。
