...
- Probabilistic 概率 y=P(y|x)：是 x 对应一组值 y 的分布。对于某一 x，y 的可能值可以是 f(x) 附近的多个。概率有更高的不确定性，而这个不确定性则是来自 noise。
...
- 性能度量（学习算法选择）：学得模型（最优函数）在数据上表现的量化形式，选取合适的函数来表示什么样子的模型是好的。也就是针对每个假设 h 的评估，用来判断该假设是否与训练集匹配。在机器学习过程中，有多种学习算法可以选择，同一个学习算法也有众多参数（超级参数）可以选择。学习算法的选择是通过对候选学习算法学得的模型的泛化性能进行评估，选择泛化性能最好的那个模型对应的学习算法及超参。对模型的泛化性能进行评估，需要有衡量泛化性能的评价标准，也就是性能度量。这个过程可以看到模型如何对尚未看到的数据做预测的，这意味着代表模型对潜在样本的表现。
- 优化方式：对性能度量进行求解，找出最合适的解，来确定最终的模型（最优函数）。
...
样本数足够大，$E_{in}(h)≈E_{out}(h)$ 就会成立，不会受 D 好坏的影响。
...
性能度量（performance measure）是在训练中用来对候选模型（假设 h）的性能进行的评估方式，然后选择性能最好（泛化误差最小）的那个假设作为最优函数 g。它是关于真实值和预测值的关系。真实值与预测值越接近，或者说真实的分布与预测分布越接近，性能越好。

##### 分类问题

- 0/1 Error：$err(\hat y,y)=P[\hat y \neq y]$
- 准确率/错误率：
...
机器学习中用损失函数来评估假设 h 针对某一个样本的性能，也就是计算模型预测值与真实值的差距，从而确定优化的方向。损失函数必须是可求梯度的，就是可以求偏导数的，这样比较容易被优化。

- 损失函数（Loss Function）：指在单个样本上的误差，用来评估模型一次预测的好坏。

- 代价函数（Cost Function）：又叫成本函数或经验风险，是对训练集所有样本点的损失函数的平均值的最小化。仅仅经验风险最小化是不行的，这样容易导致过拟合。因此不仅要让经验风险最小化，还要考虑模型复杂度，让结构风险最小化。人们习惯上所说的损失函数其实有时候是指“代价函数”。

  损失函数用于衡量模型拟合的程度，越小就代表拟合得越好。

- 目标函数（Object Function）：指最终需要优化的函数，也就是结构风险=经验风险+正则项（惩罚项）。
...
损失函数也与性能度量类似，真实值与预测值差别越大、Loss越大，优化的目标就是减小Loss。从评估的角度来说，性能度量是整体目标，而损失函数是具体优化方向，两者可以相同也可以不同。常见的 MSE 均方误差或 logLoss 既可以被当作性能度量，也是回归问题的损失函数。但在更多的问题中，经常发现为了减小模型的错误率（性能度量），并不直接优化错误率而是会优化另一个函数（损失函数）。比如：

- 分类问题的损失函数可以是 Cross Entropy，但性能度量可以是 accuracy recall
- 在逻辑回归中，会优化对数似然
- 在SVM中，会优化hinge loss
- 在adaboost中会优化指数损失。
